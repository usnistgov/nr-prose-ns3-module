...
main(int argc, char* argv[])
{
    ...
    for (uint32_t u = 0; u < ueVoiceContainer.GetN(); ++u)
    {
        ...
        // the direct link
        ipv4AddressVector[u] =
            ueVoiceContainer.Get(u)->GetObject<Ipv4L3Protocol>()->GetAddress(1, 0).GetLocal();
    }
    ...
    for (uint32_t i = 0; i < ueVoiceContainer.GetN() - 1; ++i)
    {
        for (uint32_t j = i + 1; j < ueVoiceContainer.GetN(); ++j)
        {
            ...
            ApplicationContainer app =
                sidelinkClient.Install(ueVoiceContainer.Get(i)); // Installed in UE i
            Time appStart = startTrafficTime + Seconds(startTimeRnd->GetValue());
            app.Start(appStart);
            clientApps.Add(app);
            NS_LOG_INFO("OnOff application installed in UE nodeId "
                        << i << " srcIp " << ipv4AddressVector[i] << " towards UE nodeId " << j
                        << " dstIp " << ipv4AddressVector[j]);
            ...
            if (bidirectional)
            {
                ...
                sidelinkClient.SetConstantRate(DataRate(dataRateString), udpPacketSize);
                ApplicationContainer app =
                    sidelinkClient.Install(ueVoiceContainer.Get(j)); // Installed in UE j
                Time appStart = startTrafficTime + Seconds(startTimeRnd->GetValue());
                app.Start(appStart);
                clientApps.Add(app);
                NS_LOG_INFO("OnOff application installed in UE nodeId "
                            << j << " srcIp " << ipv4AddressVector[j] << " towards UE nodeId " << i
                            << " dstIp " << ipv4AddressVector[i]);
    ...
    NS_LOG_DEBUG("Used " << streamsUsed << " random variable streams in ApplicationHelper");
    ...
    *Pc5SignallingPacketTraceStream->GetStream()
    ...
    std::cout << "System average thput = "
              << (rxByteCounter * 8) / (simTime - startTrafficTime).GetSeconds() / 1000.0 << " kbps"
              << std::endl;
         ...
         ++i)
    {
        ...
        outFile << "    Tx Data rate: " << i->second.txBytes * 8.0 / appDuration / 1000
                << " kbps\n";
        outFile << "    Rx Packets:   " << i->second.rxPackets << "\n";
        outFile << "    Rx Bytes:     " << i->second.rxBytes << "\n";
        if (i->second.rxPackets > 0)
        {
            outFile << "    Rx Data rate: " << i->second.rxBytes * 8.0 / appDuration / 1000
                    << " kbps\n";
            outFile << "    Mean delay:   "
