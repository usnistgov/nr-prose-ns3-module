...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < gNbNodes.GetN (); ++i)
    {
      ...
      for (uint32_t j = 0; j < relayUeNodes.GetN (); j++)
        {
          std::cout << "Relay " << j << " initial distance from gNB " << i << " is " << relayUeNodes.Get (j)->GetObject<MobilityModel> ()->GetDistanceFrom (gNbMobility) << std::endl;
        }
    }
  ...
  for (uint32_t i = 0; i < relayUeNodes.GetN (); ++i)
    {
      ...
      for (uint32_t j = 0; j < remoteUeNodes.GetN (); j++)
        {
          std::cout << "Remote UE " << j << " initial distance from relay " << i << " is " << remoteUeNodes.Get (j)->GetObject<MobilityModel> ()->GetDistanceFrom (relayUeMobility) << std::endl;
        }
    }
   ...
   * fix MCS value (as NrSlUeMacSchedulerDefault) and schedules logical channels
