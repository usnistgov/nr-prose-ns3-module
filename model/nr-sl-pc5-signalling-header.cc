...
static const std::vector<std::string> s_acronyms{"None",
                                                 "PDL-Es-Req",
                                                 "PDL-Es-Ac",
                                                 "PDL-Es-Rj",
                                                 "PDL-Mo-Req",
...
static const std::vector<std::string> s_names{"None",
...
/*****     NrSlPc5SignallingMessageType : To peek at the first byte of the received message and
 * identify the type       *****/
...
ProseDirectLinkEstablishmentRequest::Print(std::ostream& os) const
{
    os << "msgId: " << (uint16_t)m_msgId << " "
       << "seqNum: " << +m_seqNum << " ";
    os << " proseAppIds: ";
    for (std::vector<uint32_t>::const_iterator it = m_proseAppIds.begin();
         it != m_proseAppIds.end();
         ++it)
    {
        os << (*it) << " ";
    }
    ...
    os << "secCapabilities: ";
    for (std::vector<uint8_t>::const_iterator it = m_secCapabilities.begin();
         it != m_secCapabilities.end();
         ++it)
    {
        os << +(*it) << " ";
    }
...
{
    uint32_t size = 0;
    size = size + sizeof(m_msgId) + sizeof(m_seqNum) + 1 // ProSe application identifier IEI octet
    ...
    if (m_hasKeyEsInfoContainer)
    {
        size = size + 1 // Key establishment information container IEI octet
    ...
    if (m_hasNonce1)
    {
        size = size + 1     // Nonce IEI octet
               + NonceSize; // Nonce IEI contents
    }
    if (m_hasMsb)
    {
        size = size + 1         // Most significant bits of the KNRP-sess ID IEI octet
               + sizeof(m_msb); // Most significant bits of the KNRP-sess ID IEI contents
    }
    if (m_hasTgtUserInfo)
    {
        size = size + 1 // Application layer ID IEI octet (Target user info)
    ...
    if (m_hasKnrpId)
    {
        size = size + 1            // Knrp ID IEI octet
               + sizeof(m_knrpId); // Knrp ID content
    }
    if (m_hasRelayServiceCode)
    {
        size = size + 1 // Relay service code IEI octet
...
ProseDirectLinkEstablishmentRequest::Serialize(Buffer::Iterator start) const
{
    ...
    i.WriteU8(m_proseAppIds.size() *
              sizeof(uint32_t)); // Length of ProSe application identifier content octet
    ...
    i.WriteU8(0); // Application layer ID IEI octet (Source user info) //TODO: Not defined in the
                  // standard ATM
    i.WriteU8(
        sizeof(m_srcUserInfo)); // Length of Application layer ID content octet (Source user info)
    ...
    i.WriteU8(m_secCapabilities.size() *
              sizeof(uint8_t)); // Length of UE security capabilities  content octet
    ...
    i.WriteU8(0); // UE PC5 unicast signalling security policy IEI octet //TODO: Not defined in the
                  // standard ATM
    ...
    if (m_hasKeyEsInfoContainer)
    {
        ...
        i.WriteU8(sizeof(m_keyEsInfoContainer)); // Length of Key establishment information
                                                 // container content octet
    ...
    if (m_hasTgtUserInfo)
    {
        ...
        i.WriteU8(sizeof(
            m_tgtUserInfo)); // Length of Application layer ID content octet (Target user info)
    ...
    if (m_hasRelayServiceCode)
    {
        i.WriteU8(25); // Relay service code IEI octet //TODO: Not defined in the standard ATM,
                       // using LTE value
...
void
ProseDirectLinkEstablishmentRequest::SetParameters(uint8_t seq,
...
ProseDirectLinkEstablishmentAccept::Print(std::ostream& os) const
{
    ...
    for (std::vector<uint8_t>::const_iterator it = m_qosFlowDescriptions.begin();
         it != m_qosFlowDescriptions.end();
         ++it)
    {
        os << +(*it) << " ";
    }
...
{
    uint32_t size = 0;
    size = size + sizeof(m_msgId) + sizeof(m_seqNum) + 1 // Source user info IEs
           ...
           + sizeof(m_userPlaneSecConfig); // Configuration of UE PC5 unicast user plane security
                                           // protection contents

    if (m_hasIpAddressConfig)
    {
        size = size + 1                     // IP address configuration IEI
               + sizeof(m_ipAddressConfig); // IP address configuration contents
    }
    if (m_hasLinkLocalIpv6Address)
    {
        size = size + 1                    // link-local IPv6 address IEI
...
ProseDirectLinkEstablishmentAccept::Serialize(Buffer::Iterator start) const
{
    ...
    i.WriteU16(m_qosFlowDescriptions.size() *
               sizeof(uint8_t)); // PC5 QoS flow descriptions contents length
    ...
    i.WriteU8(0); // Configuration of UE PC5 unicast user plane security protection IEI //TODO: Not
                  // defined in the standard ATM
    i.WriteU8(m_userPlaneSecConfig); // Configuration of UE PC5 unicast user plane security
                                     // protectio contents
...
ProseDirectLinkEstablishmentAccept::Deserialize(Buffer::Iterator start)
{
    ...
    m_userPlaneSecConfig =
        i.ReadU8(); // Configuration of UE PC5 unicast user plane security protection contents
...
void
ProseDirectLinkEstablishmentAccept::SetParameters(uint8_t seq,
                                                  uint32_t srcUserInfo,
...
void
ProseDirectLinkEstablishmentAccept::SetPc5QoSFlowDescriptions(
    std::vector<uint8_t> qosFlowDescriptions)
{
    m_qosFlowDescriptions.clear();
...
void
ProseDirectLinkEstablishmentAccept::SetUserPlaneSecurityProtectionConfiguration(
    uint8_t userPlaneSecConfig)
{
...
void
ProseDirectLinkEstablishmentAccept::SetLinkLocalIpv6Address(
    std::vector<uint8_t> linkLocalIpv6Address)
{
    m_hasLinkLocalIpv6Address = true;
    m_linkLocalIpv6Address.clear();
...
{
    return sizeof(m_msgId) + sizeof(m_seqNum) + sizeof(m_pc5SignallingProtocolCause);
}
...
void
ProseDirectLinkEstablishmentReject::SetPc5SignallingProtocolCause(
    uint8_t pc5SignallingProtocolCause)
{
...
{
    uint32_t size = 0;
    size = size + sizeof(m_msgId) + sizeof(m_seqNum) + sizeof(m_pc5SignallingProtocolCause) +
           sizeof(m_msbKnrpId);
    if (m_hasBackoffValue)
    {
